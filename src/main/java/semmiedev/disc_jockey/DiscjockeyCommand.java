    ...
    public static void register(CommandDispatcher<FabricClientCommandSource> commandDispatcher) {
        ...
        commandDispatcher.register(
                literal("discjockey")
                        ...
                        .then(literal("info")
                                .executes(context -> {
                                    ...
                                    if (!Main.SONG_PLAYER.tuned) {
                                        ...
                                        context.getSource().sendFeedback(Component.translatable(Main.MOD_ID + ".info_playing", formatTimestamp((int) 0), formatTimestamp((int) Main.SONG_PLAYER.song.length), Main.SONG_PLAYER.song.displayName, Main.SONG_PLAYER.speed));
                                        return 0;
                                    }else {
                        ...
                        .then(literal("loop")
                                .executes(context -> {
                                    String mode = SongPlayer.loopMode == 0 ? "off" : (SongPlayer.loopMode == 1 ? "playlist" : "single");
                                    context.getSource().sendFeedback(Component.translatable(Main.MOD_ID + ".loop_status", mode));
                                    return 1;
                                })
                                .then(literal("off")
                                    .executes(context -> {
                                        SongPlayer.loopMode = 0;
                                        context.getSource().sendFeedback(Component.translatable(Main.MOD_ID + ".loop_disabled"));
                                        return 1;
                                    }))
                                .then(literal("playlist")
                                    .executes(context -> {
                                        SongPlayer.loopMode = 1;
                                        context.getSource().sendFeedback(Component.translatable(Main.MOD_ID + ".loop_enabled"));
                                        return 1;
                                    }))
                                .then(literal("single")
                                    .executes(context -> {
                                        SongPlayer.loopMode = 2;
                                        context.getSource().sendFeedback(Component.translatable(Main.MOD_ID + ".loop_enabled"));
                                        return 1;
                                    }))
                        )
